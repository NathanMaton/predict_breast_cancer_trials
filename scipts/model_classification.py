...
class ClassificationModel():
    ...
    def timedelta_change(self, x):
        '''
        Applies a change to the days to a float, there's a mix of datetime Objects
         and floats in the dataframe column length
        '''
        try:
            y = x.days
        except:
    ...
    def apply_length_adjustment(self):
        ...
        self.df_data[label_col_name] = self.df_data[label_col_name].apply(
            self.timedelta_change)
        self.df_data.fillna(0, inplace=True)

    def drop_bad_columns(self):
        '''
        Drops these trial status column because it leaks information to the target variable
        '''
        bad_columns = 'Completed|Discontinued|Other Trial Status'
        bad_column_names = self.df_data.filter(
            regex=bad_columns).columns.tolist()
    ...
    def test_train_split(self, random_state=42, test_size=0.2):
        ...
        Preps the dat to all be floats
        Performs a train test split
        '''

        # Drops leaky information
        ...
        if self.df_data.filter(regex='Pass').shape[1] != 1:
            logger.info(
                'More than one pass label column - adjust column names')
            return
    ...
    def logistic_regression_model(self):
        ...
        self.param_grid = {
            ...
            'logistic__C': np.logspace(-4, 2, 30),
        }
    ...
    def xgboost_model(self):
        ...
        self.param_grid = {
            ...
            'XGB__n_estimators': [40, 50, 70, 80, 90, 100],
    ...
    def predict_test(self):
        ...
        # Calculates the predict probability. Had to make sure each model
        # was using a log loss if it was not the default loss function
        self.predict_prob = self.model_gscv.best_estimator_.predict_proba(
            self.X_test)

        # Calculate the naive prediction based off of the mean training target
        self.naive_accuracy_preds = np.ones(
            self.y_test.shape[0])*np.round(self.y_train.mean(), 0)
        ...
        # Naive accuracy score - guessing the mean
        self.naive_accuracy_score = accuracy_score(
            y_true=self.y_test, y_pred=self.naive_accuracy_preds)
        ...
        # Builds the naive log loss score
        self.naive_log_loss_preds = np.ones(
            self.y_test.shape[0])*self.y_train.mean()
        self.naive_log_loss_score = np.exp(-1*log_loss(
            y_true=self.y_test, y_pred=self.naive_log_loss_preds))
        ...
        logger.info(
            f'Naive Accuracy Test Score: {np.round(self.naive_accuracy_score,3)}')
        logger.info(
...
if __name__ == '__main__':
    ...
    model1 = ClassificationModel(
        df_data=df_data1, model_type='logistic_regression')
